...
exports.validateRWPerm = ({
  ...
  if (
    (newItem &&
      Object.prototype.hasOwnProperty.call(newItem, Globals.ownerFieldName) &&
      user === newItem[Globals.ownerFieldName]) ||
    (list &&
      Object.prototype.hasOwnProperty.call(list, Globals.ownerFieldName) &&
      user === list[Globals.ownerFieldName])
  ) {
    return true;
  }
  ...
  if (
    list &&
    Object.prototype.hasOwnProperty.call(list, Globals.readWritePermFieldName)
  ) {
  ...
  if (
    ...
    const splittedRW =
      list[Globals.itemReadWritePermFieldName].split(/\s*,\s*/);
  ...
  if (
    ...
    const splittedRW = newItem[Globals.readWritePermFieldName].split(/\s*,\s*/);
    if (readWrite) {
      mergedPerm = mergedPerm.concat(splittedRW);
    } else {
      mergedPerm = splittedRW;
  ...
  //   includes the user
  if (
    ((!readWrite || (readWrite && acceptAll)) &&
      mergedPerm.includes(Globals.allUserName)) ||
    ((mergedPerm.includes(Globals.authUserName) ||
      mergedPerm.includes(Globals.allUserName)) &&
      user !== Globals.allUserName) ||
...
exports.validateRPerm = ({
  ...
  const newList = { ...list };
  if (list) {
    if (Object.prototype.hasOwnProperty.call(list, Globals.readPermFieldName)) {
      newList[Globals.readWritePermFieldName] = list[Globals.readPermFieldName];
    } else {
      delete newList[Globals.readWritePermFieldName];
    }
    if (
      Object.prototype.hasOwnProperty.call(list, Globals.itemReadPermFieldName)
    ) {
      newList[Globals.itemReadWritePermFieldName] =
        list[Globals.itemReadPermFieldName];
    } else {
  ...
  const newItem = { ...item };
  if (item) {
    if (Object.prototype.hasOwnProperty.call(item, Globals.readPermFieldName)) {
...
exports.validateDPerm = ({
  ...
  if (
    item &&
    Object.prototype.hasOwnProperty.call(item, Globals.ownerFieldName)
  ) {
  ...
  if (
    exports.validateRWPerm({
      ...
      list:
        ...
        Object.prototype.hasOwnProperty.call(
          list,
          Globals.itemCreatePermFieldName
        )
          ? {
              ...list,
              [Globals.itemReadWritePermFieldName]:
                list[Globals.itemCreatePermFieldName],
            }
          : list,
      item,
      acceptAll: !!list[Globals.itemCreatePermFieldName],
      throwError: false,
    })
  ) {
    return true;
