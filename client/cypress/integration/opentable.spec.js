...
describe('opentable basic tests', () => {
    ...
    it('makes the login form appears and disappears', () => {
    ...
    it('makes the login form appears and enter an invalid password', () => {
        cy.contains('First view').dblclick();
        cy.get('input').last().focus().type('{end}s{enter}');
        cy.get('#loginform').should('be.visible');
    ...
    it('makes the login form appears, enter a valid password and check the edited value', () => {
        cy.contains('First view').dblclick();
        cy.get('input').last().focus().type('{end}s{enter}');
        cy.get('#loginform').should('be.visible');
        cy.get('#loginhelper').should('not.contain', 'Invalid email or password...');
