...
const theme = createTheme({
  palette: {
    primary: {
      ...
      palebg: (thm) => lighten(thm.palette.primary.light, 0.9),
    },
    secondary: {
...
function App({ initialViewid, appid }) {
  const [viewid, setViewId] = React.useState(
    BrowserHistory.getViewIdFromURL(appid)
      ? BrowserHistory.getViewIdFromURL(appid)
      : initialViewid
  );
  ...
  const handleChangeViewId = React.useCallback(
    (newViewid, leaveConfigPanelOpen) => {
      ...
      if (
        leaveConfigPanelOpen === undefined ||
        leaveConfigPanelOpen === false
      ) {
      ...
      if (viewData) {
        ...
        setViewData({
          ...
          [Globals.childlistFieldName]: Utils.objWithout(
