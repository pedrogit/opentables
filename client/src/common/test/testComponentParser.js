...
describe('testComponentParser.js List Item ComponentParser', () => {
  ...
  it('Test an empty string', () => {
    expect(function(){new ComponentParser('');}).to.throw(NodeUtil.format(Errors.ErrMsg.ComponentParser_Invalid, '"[[]]"'));
  });
  ...
  it('Test a simple property name', () => {
    ...
    expect(componentParser).to.deep.equal({component: {prop3: true}, targetProp: 'prop3', control: 'text'});
  });

  it('Test an invalid property name', () => {
    var cmpStr = 'prop2+name';
    expect(function(){new ComponentParser(cmpStr);}).to.throw(NodeUtil.format(Errors.ErrMsg.ComponentParser_Invalid, '"[[' + cmpStr + ']]"'));
  });
  ...
  it('Test with a control type', () => {
    ...
    expect(componentParser).to.be.an('object');
    expect(componentParser.component).to.deep.equal({prop1: {control: 'text'}});
