...
describe("1 testSchemaValidator.js List Item Schema", () => {
  ...
  it("1.7 Validate a string value against a one level schema of type number", async () => {
    ...
    await expectThrowsAsync(
      () => schemaValidator.validateJson('{"field1": "toto"}'),
      NodeUtil.format(
  ...
  it("1.11 Validate a missing field", async () => {
    ...
    await expectThrowsAsync(
      ...
      NodeUtil.format(Errors.ErrMsg.SchemaValidator_MissingProp, "field1")
    );
  });
  ...
  it("1.13 Invalid type (number instead of string)", async () => {
    ...
    await expectThrowsAsync(
      () => schemaValidator.validateJson('{"field1": 123}'),
      NodeUtil.format(
        Errors.ErrMsg.SchemaValidator_InvalidType,
        "field1",
        "123",
  ...
  it("1.14 Validate upper", async () => {
    var schemaValidator = new SchemaValidator(
      "{field1: {type:string, required, upper}}"
    );
    var json = await schemaValidator.validateJson('{"field1": "toto"}');
  ...
  it("1.15 Invalid email", async () => {
    ...
    await expectThrowsAsync(
      () => schemaValidator.validateJson('{"field1": "aaaa"}'),
      NodeUtil.format(
        Errors.ErrMsg.SchemaValidator_InvalidType,
        "field1",
        "aaaa",
  ...
  it("1.17 Invalid user_array", async () => {
    ...
    await expectThrowsAsync(
      () => schemaValidator.validateJson('{"field1": "aa, bb"}'),
      NodeUtil.format(
        Errors.ErrMsg.SchemaValidator_InvalidType,
        "field1",
        "aa, bb",
