...
class SchemaValidator {
  ...
  async traverse(obj, parentKey = null, level = 0, callbacks = null) {
    for (var key in obj) {
      ...
      if (!(callbacks[level] === null) && typeof callbacks[level] === 'function') {
  ...
  async validateJson(jsonstr, strict = true) {
    ...
    if (typeof jsonstr === 'string') {
      try {
        ...
        throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_Malformed, err.message));
      }
    }
    else {
      json = jsonstr;
    ...
    if (strict) {
      ...
      if (!(jsonkeys === null) && 
            !(jsonkeys.length === 0) && 
            !(this.schema.getRequired().every(field => {var inc = jsonkeys.includes(field); if (!inc) {missingField = field}; return inc}))) {
              throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_MissingProp, missingField));
      }
    }
  ...
  async validateField(key, obj, parentKey) {
    if (!(Object.keys(this.schema.schema).includes(key))) {
      throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_InvalidProp, key));
    };
  ...
  validate_type_objectid(key, val) {
    ...
    if (!(MongoDB.ObjectId.isValid(val))) {
      throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_InvalidType, key, val, 'objectid'));
    }
  ...
  validate_type_embedded_listid(key, val) {
    if (!(MongoDB.ObjectId.isValid(val))) {
      throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_InvalidType, key, val, 'embedded_listid'));
    }
    return MongoDB.ObjectId(val);
  }
  ...
  validate_type_embedded_itemid(key, val) {
    ...
    if (!(MongoDB.ObjectId.isValid(val))) {
      throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_InvalidType, key, val, 'embedded_itemid'));
    }
    return MongoDB.ObjectId(val);
  }
  ...
  validate_type_embedded_itemid_list(key, val) {
    ...
    try {
      ...
      throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_InvalidType, key, val, 'user_array'));
    }
    return val;
  }
  ...
  validate_type_user(key, val) {
    ...
    try {
      ...
      throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_InvalidType, key, val, 'user'));
    }
    return val;
  }
  ...
  validate_type_string(key, val) {
    if (typeof val !== 'string') {
      throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_InvalidType, key, val, 'string'));
    }
    return val;
  }
   ...
   async validate_type_encrypted_string(key, val) {
    if (typeof val !== 'string') {
      throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_InvalidType, key, val, 'encrypted_string'));
    }
  ...
  validate_type_number(key, val) {
    if (typeof val !== 'number') {
      throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_InvalidType, key, val, 'number'));
    }
    return val;
  }
  ...
  validate_type_email(key, email) {
    const emailError = new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_InvalidType, key, email, 'email'));
  ...
  async validate_unique(type, key, val) {
    // search for an identique value
      ...
      if (item){
        throw new Error(NodeUtil.format(Errors.ErrMsg.SchemaValidator_NotUnique, key, val));
      }
